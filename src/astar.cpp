...
// 【可配置参数】（全部从 yaml 读取）
float target_x = 5.0f;     // 目标点 x（相对起飞点，米）
float target_y = 0.0f;     // 目标点 y（相对起飞点，米）
float target_z = ALTITUDE; // 目标点 z 高度（米）
float target_yaw = 0.0f;   // 目标偏航角（弧度）
float UAV_radius = 0.3f;   // 无人机等效半径（米）
float time_final = 70.0f;  // 超时时间（秒）
// 避障参数（全部从 yaml 读取）
float safe_margin = 0.1f;        // 安全裕度（米）
...
AvoidanceState avoidance_state = PLANNING; // mission2 内部状态机
    ...
    int max_points)
{
  ...
  std::priority_queue<std::pair<int, std::pair<int, int>>,
                      ...
                      decltype(cmp)>
      open_set(cmp);
  // g_score 缓存：栅格 ID(gx*1000+gy) → g 值
  ...
  if (goal_node.first != -1)
  {
    ...
    for (size_t i = 0; i < result_path_size; ++i)
    {
      geometry_msgs::PoseStamped pose;
      pose.header.frame_id = "map";
      pose.header.stamp = ros::Time::now();
      pose.pose.position.x = path_x[i] + init_position_x_take_off;
      pose.pose.position.y = path_y[i] + init_position_y_take_off;
      pose.pose.position.z = ALTITUDE;
      pose.pose.orientation.w = 1.0f;
      path_msg.poses.push_back(pose);
    }
    g_astar_path_pub.publish(path_msg);
    // ========== 修改结束 ==========
  }
  else
  {
    ...
    int max_points)
{
  ...
  if (!grid.world_to_grid(start_x, start_y, start_gx, start_gy) ||
      !grid.world_to_grid(goal_x, goal_y, goal_gx, goal_gy))
  {
    ROS_ERROR("[增量 A*] 起点/目标点超出地图范围（[-5,5] 米）");
    return 0;
  }
  // 边界检查：起点在障碍物内时自动偏移
  if (grid.cells[start_gx][start_gy] > 50)
  ...
  if (need_replan)
  {
    ...
    std::pair<int, int> goal_node = {-1, -1};
    int iterations = 0;
    const int MAX_ITERATIONS = 5000; // 增量 A*迭代次数减少 50%
    while (!open_set.empty() && iterations < MAX_ITERATIONS)
    {
      auto current = open_set.top();
      open_set.pop();
      ...
      int max_size)
  {
    ...
    const float CURVATURE_THRESHOLD = 0.4f; // 曲率阈值（1/半径），>0.4 视为急弯
    ...
    corridor_marker.type = visualization_msgs::Marker::LINE_STRIP;
    ...
    corridor_marker.color.g = 1.0f;
    corridor_marker.color.b = 0.0f;
    corridor_marker.color.a = 0.7f;
    corridor_marker.scale.x = 0.05f;
    corridor_marker.lifetime = ros::Duration(0.5f);
    g_corridor_pub.publish(corridor_marker);
    // ========== 修改结束 ==========
    return corridor_count;
  }
  // ============================================================================
  // 核心算法 4：增强 VFH+ 避障（完整实现）
  // ============================================================================
      ...
      bool &out_need_replan)
  {
    ...
    // 动态安全裕度：速度越快，安全裕度越大（0.2~1.0 倍基础裕度）
    float dynamic_safe_margin = safe_margin * (0.2f + 0.8f * current_speed / (max_speed + 0.1f));
    ...
    grid_msg.info.origin.position.x = drone_x - (GRID_SIZE * GRID_RESOLUTION) / 2.0f;
    grid_msg.info.origin.position.y = drone_y - (GRID_SIZE * GRID_RESOLUTION) / 2.0f;
    grid_msg.info.origin.position.z = 0;
    grid_msg.info.origin.orientation.w = 1.0f;
    grid_msg.data.resize(GRID_SIZE * GRID_SIZE);
    for (int i = 0; i < GRID_SIZE; ++i)
    {
      for (int j = 0; j < GRID_SIZE; ++j)
      {
        grid_msg.data[i * GRID_SIZE + j] = static_cast<int8_t>(certainty_grid[i][j]);
      }
    }
    g_vfh_grid_pub.publish(grid_msg);
    // ========== 修改结束 ==========
    // ========== 4. VFH+ 直方图构建（72 扇区，5°/扇区） ==========
    ...
    if (!is_oscillating && pos_history.size() >= 30)
    {
      ...
      {
        is_oscillating = true;
        oscillation_frames = 40; // 持续 2 秒（20Hz×2）
    ...
    // 5.2 目标不可达检测（连续 5 帧目标方向被阻挡）
    static int target_blocked_count = 0;
    bool target_blocked = false;
    // 射线检测：从无人机到目标的连线是否被障碍物阻挡
    float ray_steps = std::max(5.0f, dist_to_target / 0.2f); // 最少 5 步，步长 0.2m
    ...
    if (is_oscillating && oscillation_frames > 0)
    {
      ...
      float TIME_STEP = 0.1f; // 10Hz 控制频率
    ...
    if (enable_corridor && corridor_size > 0)
    {
      ...
      if (dist_to_corridor_center < half_width)
      {
        ...
        if (to_center_mag > 1e-6f)
        {
          ...
          // 权重：0.3（基础）+ 0.7×(1- 归一化距离) → 中心 0.3，边缘 1.0
    ...
    // ========== 9. 历史方向记忆 + 滞后效应 ==========
    static std::vector<int> bin_history; // 扇区历史（3 帧）
  ...
  int main(int argc, char **argv)
  {
    ...
    // ========== 修改结束 ==========
    // 设置话题发布频率，需要大于 2Hz，飞控连接有 500ms 的心跳包
    ros::Rate rate(20);
    // ========== 参数读取（yaml → 全局变量） ==========
