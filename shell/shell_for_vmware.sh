...
tmux send-keys -t $SESSION:2.1 "sleep 15; source ${MAIN_WS}/devel/setup.zsh; roslaunch astar astar.launch" C-m

# Pane 2.2: YOLO 圆环检测
tmux split-window -h -t $SESSION:2.1
tmux send-keys -t $SESSION:2.2 "sleep 12; source ${MAIN_WS}/devel/setup.zsh; rosrun astar ring_detector.py" C-m
...
tmux send-keys -t $SESSION:3.0 "sleep 10; source ${LAND_WS}/devel/setup.zsh; roslaunch scan_land scan_land_py.launch" C-m

# Pane 3.1: 视觉起降主节点 (后启动，有 7s 间隔)
tmux split-window -v -t $SESSION:3.0
tmux send-keys -t $SESSION:3.1 "sleep 17; source ${LAND_WS}/devel/setup.zsh; roslaunch scan_land scan_land.launch" C-m
